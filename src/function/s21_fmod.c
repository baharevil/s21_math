...
/*void bin_double(double x) {
  ...
  for (; i != 31; i--, x754.ieee.mantissa1 >>= 0x1) {
    *(arr + i) = x754.ieee.mantissa1 & 0x1;
  }

  for (; i != 11; i--, x754.ieee.mantissa0 >>= 0x1) {
    *(arr + i) = x754.ieee.mantissa0 & 0x1;
  }

  for (; i != 0; i--, x754.ieee.exponent >>= 0x1) {
...
long double s21_fmod(double x, double y) {
  ...
  // Любой другой вариант
    ...
    //   // bin_double(int_part.d);  // Для того чтобы посмотреть что там с
    //   битами
    ...
    //     //   Если степень больше 20, то нам понадобится вся мантисса0 и
    //     степень
    ...
    //       //     Сдвигаемся вправо на количетсво степеней (забивается 0 из-за
    //       //     правого сдвига)
    //       temp.ieee.mantissa1 >>= (degree - 20);

    //       //     Производим обратную инверсию, так чтобы 0 стали 1, а 1 стали
    //       0 temp.ieee.mantissa1 = ~temp.ieee.mantissa1;
    //     }
    ...
    //       //     Сдвигаемся вправо на количетсво степеней (все забивается 0
    //       из-за
    //       //     правого сдвига)
    //       temp.ieee.mantissa0 >>= degree;

    //       //     Производим обратную инверсию, так чтобы 0 стали 1 (это и
    //       есть
    //       //     наша целая часть), а 1 стали 0
    //       temp.ieee.mantissa0 = ~temp.ieee.mantissa0;
    //     }

    //     //   printf("\nthis is temp\n");
    //     //   bin_double(temp.d);  // Для того чтобы посмотреть что там с
    //     битами

    //     //   Производим логическое "и" чтобы убрать дробную часть и оставить
    //     //   только целую часть от деления х / у
    ...
    //   result =
    //       (double)s21_fabs(copy_x.d) - ((double)s21_fabs(copy_y.d) *
    //       int_part.d);
